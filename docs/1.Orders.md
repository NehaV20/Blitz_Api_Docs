The order APIs let you place orders of various types, modify or cancel pending orders, and retrieve daily order history. With Blitz, you can manage your trades efficiently, execute orders in real time, track their status, and maintain full control over your portfolio .
...
```bash
curl -X 'GET' \
  'http://182.70.127.254:44346/api/order' \
  -H 'accept: */*'
```
### Request Structure
- No Body
...
```bash
curl -X 'GET' \
  'http://182.70.127.254:44346/api/orders/{BlitzOrderId}' \
...
Submit a new order to the Blitz system. You can specify the instrument, order type, quantity, price, and other order parameters. This endpoint executes the order in real-time, depending on the provided details.


```bash
curl -X 'POST' \
  'http://182.70.127.254:44346/api/order/placeOrder' \
  -H 'accept: */*' \
...
The **Modify Order** endpoint allows you to update an existing order's quantity, price, stop price, or other parameters. Use this API to adjust open or pending orders before they are executed.

---

```bash
curl -X 'PUT' \
  'http://182.70.127.254:44346/api/order/modifyOrder' \
  -H 'accept: */*' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer <your_jwt_token>' \
  -d '{
...
## 5. Cancel Order {#cancel-order}
 Cancel an existing order using the `InstrumentId` and `ExchangeOrderId`.
...
```bash
curl -X 'DELETE' \
  'http://182.70.127.254:44346/api/order/cancelOrder?InstrumentId=123&ExchangeOrderId=456' \
  -H 'accept: */*' \
  -H 'Authorization: Bearer YOUR_JWT_TOKEN'
```
